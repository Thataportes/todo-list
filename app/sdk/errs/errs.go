...
// Newf constructs an error based on a formatted error message.
func Newf(code ErrCode, format string, v ...interface{}) *Error {
	pc, filename, line, _ := runtime.Caller(1)
...
// If it doesn't exist, will create one from the error.
...
func (e *Error) Equal(e2 *Error) bool {
	return e.Code.Equal(e2.Code) && e.Message == e2.Message
}
